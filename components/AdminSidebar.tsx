...
const navItems = [
  ...
  { href: '/admin/activities', label: 'Acara', icon: '🎯' },
...
export default function AdminSidebar({ username }: Props) {
  ...
  return (
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'linear-gradient(135deg,#1e3a8a,#1d4ed8)', boxShadow: '0 2px 16px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <div style={{ color: '#fff', fontSize: '14px', fontWeight: '800', fontFamily: "'Sora',sans-serif", lineHeight: 1 }}>SMKN 2 Jember — Admin</div>
              <div style={{ color: '#93c5fd', fontSize: '10px' }}>👤 {username}</div>
            </div>
          </div>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} style={{
                flex: 1, minWidth: '52px', display: 'flex', flexDirection: 'column', alignItems: 'center',
                ...
                <span style={{ fontSize: '8px', fontWeight: '700', color: isActive ? '#fff' : 'rgba(255,255,255,0.6)', letterSpacing: '0.2px' }}>{item.label}</span>
              </Link>
            );
          })}
        </div>
